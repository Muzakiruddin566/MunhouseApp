import React, { useState } from "react";
import {
  Button,
  TextInput,
  Label,
  Select,
  RangeSlider,
  Card,
} from "flowbite-react";
import {
  BiUserCircle,
  BiSolidLocationPlus,
  BiBath,
  BiBed,
  BiSolidUser,
} from "react-icons/bi";

import Navbar from "../components/buyer/Navbar";

import { Link, useNavigate } from "react-router-dom";
import FilterProperty from "../components/buyer/FilterProperty";
import HouseOwnerFooter from "../components/HouseOwnerFooter";

function Home() {
  const [advertisementList, setAdvertisementList] = useState([]);

  const navigate = useNavigate();

  const setAdvertisement = (advertisementList) => {
    setAdvertisementList(advertisementList)
  }

  return (
    <>
      <Navbar />

      <FilterProperty setAdvertisement={setAdvertisement} />

      {/* Product Card */}
      <div className="container my-10">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 items-center justify-center">
          {advertisementList &&
            advertisementList.map((advertisement, index) => (
              <div className="col" key={index}>
                <Card
                  className="max-w-sm"
                  imgAlt=""
                  imgSrc={advertisement?.media[0]}
                >
                  <Link to={`/property-details/${advertisement._id}`}>
                    <h5 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">
                      {advertisement?.address}
                    </h5>
                  </Link>

                  <div className="icon flex items-center gap-2">
                    <BiSolidUser className="text-gray-500" size={20} />
                    <h4>{`${advertisement?.seller?.first_name} ${advertisement?.seller?.last_name}`}</h4>
                  </div>

                  <div className="wrap flex justify-between items-center gap-3">
                    <div className="icon flex items-center gap-1">
                      <BiBed className="text-gray-500" size={20} />
                      <h4>{advertisement?.no_of_bedrooms} Bedroom</h4>
                    </div>

                    <div className="icon flex items-center gap-1">
                      <BiBath className="text-gray-500" size={20} />
                      <h4>{advertisement?.no_of_washrooms} Bathroom</h4>
                    </div>
                  </div>

                  <hr />

                  <div className="mb-5 mt-2.5 flex items-center justify-end">
                    <svg
                      className="h-5 w-5 text-yellow-300"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                    <svg
                      className="h-5 w-5 text-yellow-300"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                    <svg
                      className="h-5 w-5 text-yellow-300"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                    <svg
                      className="h-5 w-5 text-yellow-300"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                    <svg
                      className="h-5 w-5 text-yellow-300"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                    <span className="ml-3 mr-2 rounded bg-cyan-100 px-2.5 py-0.5 text-xs font-semibold text-cyan-800 dark:bg-cyan-200 dark:text-cyan-800">
                      5.0
                    </span>
                  </div>

                  <div className="flex items-center justify-between">
                    <span className="text-2xl font-bold text-gray-600 dark:text-white">
                      {advertisement?.price} CAD
                    </span>
                    <Button
                      onClick={() => {
                        navigate(`/property-details/${advertisement._id}`);
                      }}
                      color="light"
                    >
                      Details
                    </Button>
                  </div>
                </Card>
              </div>
            ))}
        </div>
      </div>
      <HouseOwnerFooter />
    </>
  );
}

export default Home;
